Shader "lilToon/PCSS Extension"
{
    Properties
    {
        // lilToon 2.1.9 migration compatibility
        ...
        // --- lilToon 2.1.9標準プロパティ ---
        _MainTex ("Main Texture", 2D) = "white" {}
        _Color ("Color", Color) = (1,1,1,1)
        _Cutoff ("Alpha Cutoff", Range(0.0, 1.0)) = 0.5
        
        // --- 3影システム (lilToon 2.1.9新機能) ---
        ...
        // --- SDF Face Shadow (lilToon 2.1.9新機能) ---
        ...
        // --- VRC Light Volumes 2.0.0 強化版 (UnityPCSS参考) ---
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // lilToon 2.1.9互換のためのアルファテストキーワードを追加
            ...
            // lilToon 2.1.9標準の変数やパイプライン前提に合わせた順序でPCSSインクルード
            ...
            // UnityPCSS参考: 高品質PCSSフィルタリング
            float _UsePCSSOptimized;
            float _PCSSKernelSize;
            float _PCSSBlurRadius;
            float _PCSSDepthBias;
            float _PCSSNormalBias;
            ...
            // VRC Light Volumes 2.0.0 強化版関数 (UnityPCSS参考)
            ...
            // SDF Face Shadow関数 (lilToon 2.1.9新機能)
            ...
            // lilToon 2.1.9 Fur関数 (Subdivision統一)
            float3 CalculateFur(float2 uv, float3 worldNormal, float3 worldPos)
            {
                #if defined(_USEFUR_ON)
                    // Fur Subdivision計算
                    float furLayer = fmod(worldPos.y * _FurSubdivision, 1.0);
                    float furDensity = tex2D(_FurTex, uv).r * _FurDensity;
                    
                    // 重力と風の影響
                    float3 furOffset = float3(0, _FurGravity * _FurLength, 0);
                    furOffset += float3(sin(_Time.y * _FurWindSpeed) * _FurWind * _FurLength, 0, 0);
                    
                    // Fur AOとShadow
                    float furAO = lerp(1.0, _FurAO, furLayer);
                    float furShadow = lerp(1.0, _FurShadow, furLayer);
                    
                    return float3(furDensity, furAO, furShadow);
                #else
                    return float3(1.0, 1.0, 1.0);
                #endif
            }
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // lilToon 2.1.9慣習: アルファテスト有効時はForwardでもclipを適用
                ...
                // 3影システム (lilToon 2.1.9)
                float shadow1 = 1.0;
                float shadow2 = 1.0;
                float shadow3 = 1.0;
                
                #if defined(_USEPCSS_ON)
                    // UnityPCSS参考: 高品質PCSSを使用
                        ...
                        #if defined(_USE_OPTIMIZED_PCSS_ON)
                            // UnityPCSS参考: 高品質フィルタリング
                            shadow1 = PCSS_UnityPCSS_Optimized(shadow1, i.pos.z, _PCSSBlurRadius, _LocalPCSSLightSize, samples);
                        #else
                ...
                // VRC Light Volumes 2.0.0 強化版 (UnityPCSS参考)
                    ...
                    float3 lightVolumeColor = SampleVRCLightVolumes_Enhanced(i.worldPos, i.worldNormal);
                ...
                // lilToon 2.1.9 Fur機能の適用
                #if defined(_USEFUR_ON)
                    float3 furData = CalculateFur(i.uv, i.worldNormal, i.worldPos);
                    col.rgb *= furData.y; // Fur AO
                    finalShadow *= furData.z; // Fur Shadow
                #endif
    ...
    // lilToon 2.1.9 フォールバック互換：標準トゥーンに落とす
    FallBack "lilToon"
