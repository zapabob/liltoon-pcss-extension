...
public class MaterialBackup : EditorWindow
{
            ...
            for (int i = 0; i < sharedMaterials.Length; i++)
            {
                ...
                if (material != null)
                {
                    ...
                    foreach (var kv in backupData.properties)
                    {
                        ...
                        {
                            var propType = ShaderUtil.GetPropertyType(shader, GetPropertyIndexByName(shader, kv.Key));
                            switch (propType)
                            {
                                case ShaderUtil.ShaderPropertyType.Color:
    ...
    {
        // Unity縺ｮVector4.ToString()蠖｢蠑・ (x, y, z, w)
        s = s.Trim('(', ')');
        var parts = s.Split(',');
        if (parts.Length == 4)
        {
    ...
    // 霑ｽ蜉: 繝励Ο繝代ユ繧｣蜷阪°繧峨う繝ｳ繝・ャ繧ｯ繧ｹ繧貞叙蠕励☆繧九・繝ｫ繝代・
