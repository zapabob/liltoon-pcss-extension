...
public class AdvancedLilToonModularAvatarGUI : ShaderGUI
{
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        if (showModularAvatarSettings)
        {
            ...
            DrawProperty(props, "_ModularAvatarRealtimeQuality", "Real-time Quality");
            DrawToggleProperty(props, "_ModularAvatarAutoLightManagement", "Auto Light Management");
        }
        ...
        if (showPCSSSettings)
        {
            ...
            DrawToggleProperty(props, "_UseOptimizedPCSS", "Use Optimized PCSS", "_USE_OPTIMIZED_PCSS_ON");
        }
    ...
    private void DrawToggleProperty(Dictionary<string, MaterialProperty> props, string propertyName, string displayName, string keyword = "")
    {
        if (props.ContainsKey(propertyName))
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                {
                    foreach (Material mat in currentMaterialEditor.targets)
                    {
                        if (props[propertyName].floatValue == 1.0f)
    ...
    private void SetupModularAvatarIntegration(Material material)
    {
        ...
        if (avatarRoot.GetComponent<Runtime.ModularAvatarPCSSController>() == null)
        {
            avatarRoot.AddComponent<Runtime.ModularAvatarPCSSController>();
        }
