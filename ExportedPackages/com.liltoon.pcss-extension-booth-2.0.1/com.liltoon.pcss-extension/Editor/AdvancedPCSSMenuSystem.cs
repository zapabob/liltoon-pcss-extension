...
namespace lilToon.PCSS.Editor
{
    ...
    public class AdvancedPCSSMenuSystem : EditorWindow
    {
        ...
        private bool showBatchProcessor = false;
        ...
        private void DrawPresetTab()
        {
            ...
            var integratedPresets = PCSSMenuSystem.IntegratedPresets;
        ...
        private void DrawBatchProcessingTab()
        {
            ...
            if (batchOperations.Count > 0)
            {
                ...
                if (isBatchProcessing)
                {
                    ...
                    EditorGUILayout.ProgressBar(batchProgress, $"進捗: {batchProgress:P1}");
                }
            }
        }
        ...
        {
            // カスタムプリセットを適用
            var integratedPreset = new PCSSMenuSystem.IntegratedPreset
            {
                name = preset.name,
        ...
        private void CreateCustomPresetFromPreset(PCSSMenuSystem.IntegratedPreset preset)
        {
        ...
        private void ShowPresetPreview(PCSSMenuSystem.IntegratedPreset preset)
        {
            // プリセットプレビュー表示
            Debug.Log($"プリセットプレビュー: {preset.name}");
        }
        
        private void ApplyIntegratedPreset(PCSSMenuSystem.IntegratedPreset preset)
        {
            // 既存のPCSSMenuSystemのApplyIntegratedPresetを呼び出し
            PCSSMenuSystem.ApplyIntegratedPreset(preset);
        }
