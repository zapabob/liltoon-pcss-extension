Shader "lilToon/PCSS Extension"
{
    Properties
    {
        ...
        // --- VRC Light Volumes 2.0.1 強化版 (lilToon 2.2.1対応) ---
        ...
        // --- VRC Light Volumes 2.0.1最適化設定 (lilToon 2.2.1新機能) ---
        ...
        // --- ファー機能 (lilToon 2.2.1 Subdivision統一・最適化) ---
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // VRC Light Volumes 2.1.0 強化版関数 (lilToon 2.1.10最適化対応)
            ...
            float3 SampleVRCLightVolumes(float3 worldPos, float3 worldNormal)
            {
                ...
                #else
                    // フル機能版 - ピクセル単位計算と方向性考慮 (lilToon 2.1.10最適化)
                    float3 localPos = mul(_VRCLightVolumeWorldToLocal, float4(worldPos, 1.0)).xyz;
                    float3 volumeUV = localPos * 0.5 + 0.5;
                    // ボリュームの範囲外なら影響なし
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float shadow3 = 1.0;
                    ...
                    // PCSSを使用 - 標準のシャドウ座標を使用 (lilToon 2.1.10最適化)
                ...
                // ファー機能 (lilToon 2.1.10最適化)
                #if defined(_USEFUR_ON)
                    #if defined(_USEFUROPTIMIZATION_ON)
                        // lilToon 2.1.10最適化レベルに応じたファー品質調整
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                // --- リアル影作成機能: オフセットとスケールを適用 (ShadowCaster用) ---
                float3 offsetPos = v.vertex.xyz + _RealisticShadowOffset.xyz;
                offsetPos *= _RealisticShadowScale.xyz;
